...
import { CampaignType, CampaignResourceType, DonationStatus } from '@/src/services/donationService';
...
const DONATION_STATUS_UI: Record<number, { label: string; color: string; icon: keyof typeof Ionicons.glyphMap }> = {
  [DonationStatus.Pending]: { label: 'Đang chờ thanh toán', color: '#f59e0b', icon: 'time-outline' },
  [DonationStatus.Completed]: { label: 'Thanh toán thành công', color: '#10b981', icon: 'checkmark-circle-outline' },
  [DonationStatus.Failed]: { label: 'Thanh toán thất bại', color: '#ef4444', icon: 'close-circle-outline' },
  [DonationStatus.Cancelled]: { label: 'Đã hủy', color: '#ef4444', icon: 'close-circle-outline' },
  [DonationStatus.Expired]: { label: 'Liên kết đã hết hạn', color: '#64748b', icon: 'hourglass-outline' },
  [DonationStatus.Refunded]: { label: 'Đã hoàn tiền', color: '#8b5cf6', icon: 'reload-circle-outline' },
};
...
export default function DonateScreen({ onBack }: DonateScreenProps) {
  ...
  const { mutateAsync: createCheckout, isPending: isCreatingCheckout } = useCreateDonationCheckout();
  const { data: donationStatus, isFetching: isPollingDonationStatus } = useDonationStatus(
    donationId || undefined,
    !!donationId,
  );

  const moneyGoal = useMemo(
    () => campaignSummary?.goals?.find((goal: any) => Number(goal.resourceType) === CampaignResourceType.Money),
    [campaignSummary],
  );
  ...
  const topDonors = useMemo(() => {
    ...
    return (fundContributions || [])
      ...
      .map((donor: any, index: number) => {
        ...
        return {
          name: donorNameText,
          time: donor.createdAt ? new Date(donor.createdAt).toLocaleString('vi-VN') : 'Vừa xong',
  ...
  useEffect(() => {
    ...
    if (Number(donationStatus.status) === DonationStatus.Completed) {
      showSuccessToast('Ủng hộ thành công', 'Cảm ơn bạn đã đóng góp cho chiến dịch.');
      setDonationId(null);
      return;
    }

    if (Number(donationStatus.status) === DonationStatus.Failed) {
      showErrorToast('Thanh toán thất bại', 'Giao dịch chưa hoàn tất. Vui lòng thử lại.');
      setDonationId(null);
      return;
    }

    if (Number(donationStatus.status) === DonationStatus.Cancelled) {
      showInfoToast('Bạn đã hủy thanh toán', 'Bạn có thể tạo lại checkout khi sẵn sàng.');
      setDonationId(null);
      return;
    }

    if (Number(donationStatus.status) === DonationStatus.Expired) {
      showErrorToast('Liên kết đã hết hạn', 'Vui lòng tạo lại checkout mới để tiếp tục ủng hộ.');
      setDonationId(null);
    }
  }, [donationStatus]);
    ...
    return (
        <View className="flex-1" style={{ backgroundColor: colors.background }}>
            ...
            >
                ...
                <Touchable className="flex-row items-center gap-1 rounded-full px-3 py-1.5" style={{ backgroundColor: `${colors.status.error}18` }}>
                    ...
                    <View className="rounded-xl border p-4 shadow-sm" style={{ borderColor: colors.border, backgroundColor: colors.card }}>
                        <View className="flex-row gap-4">
                            ...
                            <View className="flex-1 flex-col justify-between py-1">
                                <View>
                                    <View className="mb-1 flex-row items-center gap-1 flex-wrap">
                                        <View className="rounded-full border border-primary/20 bg-primary/10 px-2 py-0.5">
                                            <Text className="text-xs font-bold text-primary">
                                                {campaignSummary && Number(campaignSummary.type) === CampaignType.Fundraising ? 'Gây quỹ' : 'Chiến dịch'}
                                            </Text>
                                        </View>
                                        <Ionicons name="shield-checkmark" size={16} color={colors.status.completed} />
                                        {donationId && donationStatusUi ? (
                                            <View className="rounded-full px-2 py-0.5" style={{ backgroundColor: `${donationStatusUi.color}20` }}>
                                                <Text className="text-[11px] font-bold" style={{ color: donationStatusUi.color }}>
                                ...
                                <View className="flex-row items-center gap-1">
                                    ...
                                    <Text className="text-xs" style={{ color: colors.textSecondary }}>
                                        {campaignSummary ? `${campaignSummary.procurementOrderCount || 0} giao dịch/quy trình liên quan quỹ` : 'Đang tải dữ liệu quyên góp'}
                                    </Text>
                                </View>
                            </View>
                        </View>
                        <View className="mt-4 flex-col gap-2">
                            <View className="flex-row justify-between">
                                <Text className="text-sm font-bold text-primary">
                                    {campaignSummary ? `${formatCurrency(campaignSummary.totalMoneyReceived || 0)} VND` : '--'}
                                </Text>
                                <Text className="text-sm font-medium" style={{ color: colors.textSecondary }}>
                                    Mục tiêu: {moneyGoal?.targetAmount ? `${formatCurrency(moneyGoal.targetAmount)} VND` : 'Chưa cập nhật'}
                                </Text>
                            </View>
                            <View className="h-2.5 w-full overflow-hidden rounded-full" style={{ backgroundColor: colors.surface }}>
                                <View
                                    className="h-full rounded-full bg-accent"
                                    style={{ width: `${Math.max(0, Math.min(Number(moneyGoal?.progressPercent || 0), 100))}%` }}
                                />
                            </View>
                        </View>
                    </View>
                    ...
                    <View className="border-t pt-4" style={{ borderColor: colors.border }}>
                        ...
                        <View className="flex-col gap-4">
                            {isLoadingFundContributions ? (
                                ...
                                <View className="rounded-xl border border-dashed px-4 py-5" style={{ borderColor: colors.border, backgroundColor: colors.card }}>
